...
resource cognitiveServiceExisting 'Microsoft.CognitiveServices/accounts@2023-05-01' existing = if (useExistingAccount) {
  name: name
}

resource cognitiveService 'Microsoft.CognitiveServices/accounts@2023-05-01' = if (!useExistingAccount) {
  name: name
  kind: kind
  identity: identity
  location: location
...
resource cognitiveService_deployments 'Microsoft.CognitiveServices/accounts/deployments@2023-05-01' = [
  for (deployment, index) in (deployments ?? []): if (!useExistingAccount) {
...
@batchSize(1)
resource cognitiveService_deployments_existing 'Microsoft.CognitiveServices/accounts/deployments@2023-05-01' = [
  for (deployment, index) in (deployments ?? []): if (useExistingAccount) {
    parent: cognitiveServiceExisting
    name: deployment.?name ?? '${name}-deployments'
    properties: {
      model: deployment.model
      raiPolicyName: deployment.?raiPolicyName
...
resource cognitiveService_lock 'Microsoft.Authorization/locks@2020-05-01' = if (!useExistingAccount && !empty(lock ?? {}) && lock.?kind != 'None') {
...
resource cognitiveService_lock_existing 'Microsoft.Authorization/locks@2020-05-01' = if (useExistingAccount && !empty(lock ?? {}) && lock.?kind != 'None') {
  name: lock.?name ?? 'lock-${name}'
  properties: {
    level: lock.?kind ?? ''
    notes: lock.?kind == 'CanNotDelete'
...
resource cognitiveService_diagnosticSettings 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = [
  for (diagnosticSetting, index) in (diagnosticSettings ?? []): if (!useExistingAccount) {
...
resource cognitiveService_diagnosticSettings_existing 'Microsoft.Insights/diagnosticSettings@2021-05-01-preview' = [
  for (diagnosticSetting, index) in (diagnosticSettings ?? []): if (useExistingAccount) {
    name: diagnosticSetting.?name ?? '${name}-diagnosticSettings'
    properties: {
      storageAccountId: diagnosticSetting.?storageAccountResourceId
...
module cognitiveService_privateEndpoints 'br/public:avm/res/network/private-endpoint:0.8.0' = [
  for (privateEndpoint, index) in (privateEndpoints ?? []): if (!useExistingAccount) {
    name: '${uniqueString(deployment().name, location)}-cognitiveService-PrivateEndpoint-${index}'
    scope: resourceGroup(
      privateEndpoint.?subscriptionId ?? split(split(privateEndpoint.subnetResourceId, '/subscriptions/')[1], '/')[0],
      privateEndpoint.?resourceGroupName ?? split(split(privateEndpoint.subnetResourceId, '/resourcegroups/')[1], '/')[0]
    )
    params: {
...
module cognitiveService_privateEndpoints_existing 'br/public:avm/res/network/private-endpoint:0.8.0' = [
  for (privateEndpoint, index) in (privateEndpoints ?? []): if (useExistingAccount) {
    name: '${uniqueString(deployment().name, location)}-cognitiveService-PrivateEndpoint-${index}'
    scope: resourceGroup(
      privateEndpoint.?subscriptionId ?? split(split(privateEndpoint.subnetResourceId, '/subscriptions/')[1], '/')[0],
    ...
    params: {
      name: privateEndpoint.?name ?? 'pep-${last(split(cognitiveServiceExisting.id, '/'))}-${privateEndpoint.?service ?? 'account'}-${index}'
      privateLinkServiceConnections: privateEndpoint.?isManualConnection != true
        ? [
            {
              name: privateEndpoint.?privateLinkServiceConnectionName ?? '${last(split(cognitiveServiceExisting.id, '/'))}-${privateEndpoint.?service ?? 'account'}-${index}'
              properties: {
...
resource cognitiveService_roleAssignments 'Microsoft.Authorization/roleAssignments@2022-04-01' = [
  for (roleAssignment, index) in (formattedRoleAssignments ?? []): if (!useExistingAccount) {
...
resource cognitiveService_roleAssignments_existing 'Microsoft.Authorization/roleAssignments@2022-04-01' = [
  for (roleAssignment, index) in (formattedRoleAssignments ?? []): if (useExistingAccount) {
    name: roleAssignment.?name ?? guid(
      cognitiveServiceExisting.id,
        ...
        ? [
            {
              ...
              value: useExistingAccount ? cognitiveServiceExisting.listKeys().key1 : cognitiveService.listKeys().key1
            }
          ]
        : [],
        ...
        ? [
            {
              ...
              value: useExistingAccount ? cognitiveServiceExisting.listKeys().key2 : cognitiveService.listKeys().key2
            }
          ]
        : []
    )
...
output name string = useExistingAccount ? cognitiveServiceExisting.name : cognitiveService.name

@description('The resource ID of the cognitive services account.')
output resourceId string = useExistingAccount ? cognitiveServiceExisting.id : cognitiveService.id
...
output endpoints endpointsType = useExistingAccount ? cognitiveServiceExisting : cognitiveService.properties.endpoints

@description('The principal ID of the system assigned identity.')
output systemAssignedMIPrincipalId string? = useExistingAccount
  ? cognitiveServiceExisting.?identity.?principalId
  : cognitiveService.?identity.?principalId

@description('The location the resource was deployed into.')
output location string = useExistingAccount ? cognitiveServiceExisting.location : cognitiveService.location
...
output privateEndpoints array = [
  for (pe, i) in (!empty(privateEndpoints) ? array(privateEndpoints) : []): useExistingAccount
    ? {
        name: cognitiveService_privateEndpoints_existing[i].outputs.name
        resourceId: cognitiveService_privateEndpoints_existing[i].outputs.resourceId
        groupId: cognitiveService_privateEndpoints_existing[i].outputs.groupId
        customDnsConfig: cognitiveService_privateEndpoints_existing[i].outputs.customDnsConfig
        networkInterfaceIds: cognitiveService_privateEndpoints_existing[i].outputs.networkInterfaceIds
      }
    : {
