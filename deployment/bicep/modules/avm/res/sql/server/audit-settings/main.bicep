...
// Extract subscription ID and resource group name from storage account resource ID because of Bicep issue BCP420
// breaking modules with conditional scope
var storageAccountSubscriptionId = !empty(storageAccountResourceId) && isManagedIdentityInUse ? split(storageAccountResourceId, '/')[2] : subscription().subscriptionId
var storageAccountResourceGroupName = !empty(storageAccountResourceId) && isManagedIdentityInUse ? split(storageAccountResourceId, '/')[4] : resourceGroup().name
