...
param storageAccountName string = 'kmstorage${replace('${suffix}${env}','-', '')}' //'storage${uniqueString(resourceGroup().id)}'

@description('The name of the Container in Azure Storage.')
param storageBlobContainerName string = 'smemory'
param fileImportContainerName string = 'fileupload-staging'

@description('The name of the Queue in Azure Storage.')
param externalTasksQueueName string = 'km-queue-${suffix}-${env}'
...
resource roleAssignment1 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid('Storage Queue Data Contributor-${suffix}-${env}', storageAccountName)
...
resource roleAssignment2 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid('Storage Blob Data Contributor-${suffix}-${env}', storageAccountName)
  scope: storageAccount
  properties: {
    roleDefinitionId: subscriptionResourceId(
      'Microsoft.Authorization/roleDefinitions',
...
resource roleAssignment22 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid('Storage Blob Data Owner-${suffix}-${env}', storageAccountName)
  scope: storageAccount
  properties: {
    roleDefinitionId: subscriptionResourceId(
      'Microsoft.Authorization/roleDefinitions',
...
resource roleAssignment3 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid('Storage Queue Data Message Sender-${suffix}-${env}', storageAccountName)
  scope: storageAccount
  properties: {
    roleDefinitionId: subscriptionResourceId(
      'Microsoft.Authorization/roleDefinitions',
...
resource roleAssignment4 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  name: guid('// Storage Queue Data Message Processor-${suffix}-${env}', storageAccountName)
  scope: storageAccount
  properties: {
    roleDefinitionId: subscriptionResourceId(
      'Microsoft.Authorization/roleDefinitions',
...
resource blobContainerFileImport 'Microsoft.Storage/storageAccounts/blobServices/containers@2021-09-01' = {
  parent: storageBlobService
  name: fileImportContainerName
}
